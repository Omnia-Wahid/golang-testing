...
func TestMain(m *testing.M) {
	...
	os.Exit(m.Run()) //so all other testcases in this file can run
}
func TestGetCountryRestClientError(t *testing.T) {
	rest.FlushMockups() //refresh all the mocks and adds the new one
...
func TestGetCountryInvalidErrorInterface(t *testing.T) {
	...
	rest.AddMockups(&rest.Mock{
		...
		HTTPMethod:   http.MethodGet,
		RespHTTPCode: http.StatusNotFound,
		RespBody:     `{"message": "invalid error interface when getting country AR","error": "not_found","status": "404","cause": []}`,
	})
	country, err := GetCountry("AR")
...
//don't forget to mock the request body in case of http method post
//rest.AddMockups(&rest.Mock{
//URL: "https://api.mercadolibre.com/countries/AR",
